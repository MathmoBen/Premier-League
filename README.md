# Premier-League
We experiment with training XGBoost models to predict the outcomes of games in the English Premier League. The basline model (predicting the score will be 2-1) has a success rate of around 8.4%.

- The file Premiere_League_Regression approaches the problem as one a pair of regression problems: one for the home score and one for the away score. It predicts the correct score around 11.4% of the time.
